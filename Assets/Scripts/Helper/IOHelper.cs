...
public static class IOHelper<T> {
    ...
    public static void ToTextFile(string text, string filePath)
    {
        StreamWriter writer = new StreamWriter(filePath);
        writer.Write(text);
        writer.Close();
    ...
    public static string[] LoadTextFile(string filePath, string delimiter)
    {
        if (File.Exists(filePath))
        {
            using (StreamReader sr = new StreamReader(filePath, Encoding.Default))
            {
                ...
                foreach (var line in sr.ReadLines(delimiter.ToCharArray()))
                    lines.Add(line);
